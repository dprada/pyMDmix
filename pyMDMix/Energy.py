...
class EnergyConversion(object):
    ...
    def calcPDBRatio(self, pdb, unit, refunit='WAT'):
        ...
        if not unitnum or not refnum:
            raise EnergyConversionError("Unit %s or refunit %s not found in pdb %s"%(unit,\
                                                                                     refunit,\
                                                                                     pdb.source))
    ...
    def calcReplicaExpectedValue(self, replica, probe, numsnaps=False, stepselection=[], gridspacing=S.GRID_SPACING):
        ...
        if probe in solv.comprobes.keys(): unit = solv.comprobes[probe].name
            ...
            raise EnergyConvertError("Probe %s not found for solvent in replica %s"%(probe,\
                                                                                     replica.name))
    ...
    def convert(self, replicalist, probelist=[], average=False, dg0correct=True, inprefix=None,
        ...
        for r in replicalist:
            if not isinstance(r, Replica): raise EnergyConversionError("Wrong type %s. Expected\
                                                                       Replica type."%type(r))
        ...
        if len(set(solvents)) > 1 and average:
            raise EnergyConversionError("Cannot average grids from replicas run with different\
                                        solvents: %s"%replnames)

        # Check probes
        if probelist and not set(probelist) < set(probes):
            raise EnergyConversionError("Some selected probes are not found in replica list:\
                                        %s"%list((set(probelist) - set(probes))))
